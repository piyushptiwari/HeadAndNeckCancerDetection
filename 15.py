...
from xtract_features.extract import feature_dict_from_imgarray, get_df_from_path
...
#Testing start
data_path = "C:\\Users\\peace\\Desktop\\12\\HGJ\\HN-HGJ-001\\CT\\image\\"
rtss_path='C:\\Users\\peace\\Desktop\\12\\HGJ\\HN-HGJ-001\\CT\\RTS\\'
...
print('168 reprent the number of segments and 512x512 shows the size of each segment \n',a.shape)
d='C:\\Users\\peace\\Desktop\\12\\HGJ\\'
folders = list(filter(lambda x: os.path.isdir(os.path.join(d, x)), os.listdir(d)))
print('\nPrint All available patients folder name:\n ',folders)
print('\nPrinting first 10 dcm files from first patient:\n ',ids[:10])
...
print("\nPrint shape of the dicom images of first patient: 168 represent number of images and 512x512 reprsent the size of each image \n",n_array.shape)
...
for f in range(0,len(folders)):
    data_path = d+str(folders[f])+'\\CT\\image\\'
    rtss_path=d+str(folders[f])+'\\CT\\RTS\\'
    masks_path=d+str(folders[f])+'\\CT\\masks\\'
    overlayed_path=d+str(folders[f])+'\\CT\\overlayed\\'
    jpg_path=d+str(folders[f])+'\\CT\\jpg\\'
    masked_path=d+str(folders[f])+'\\CT\\masked\\'
    
    g = glob(data_path + '/*.dcm')
    data_dict = mat73.loadmat(rtss_path+'RTSS.mat')
    data = np.array(data_dict)
    a=data_dict['contours']['Segmentation'].astype(np.uint64)
    lst_numpy_arrs, ids = extract_img_array(data_path, getID=True)
    ...
    patient = load_scan(data_path)
    imgs = get_pixels_hu(patient)
    np.save('output_csv\\'+"fullimages"+ide+".npy" , imgs)
    file_used=('output_csv\\'+"fullimages"+ide+".npy")
    n_array = np.load(file_used).astype(np.float64)
    ...
    '''
    #This code does not require to create the 4 diffrent folders for each patient,need only one folder for all patients feature extraction
    files = glob('any_empty_folder1\\'+'*.jpg')
    for f in files:
        os.remove(f)
    for i in range(0,len(g)):
        j=a[i]
        if sum(sum (j))!=0:
            c=a[i]*n_array[i]
            plt.imsave('any_empty_folder1\\'+str(i)+'masked.jpg',c)
            plt.imshow(c, interpolation='None', cmap='gray',alpha=0.5)
            #plt.show()
    X_data = []
    masked_files = glob('any_empty_folder1\\'+'*.jpg')
    ...
    print("Extracted features for masked images for patient's ID : "+folders[f])
    df.to_csv('output_csv\\'+str(folders[f])+'.csv')
    '''
    
    '''
    #features for all dicom images for experiment purpose
    ...
    '''
    #from dcm images features without id for experiment purpose
    ...
    '''
    #features for overlayed images for experiment and visualization purpose we need to run this part of code to get overlayed images and their features
    ...
    df.to_csv('output_csv\\'+str(folders[f])+'.csv')
    '''
final = 'C:\\Users\\peace\\Desktop\\12\\output_csv\\*.csv'
df = pd.concat((pd.read_csv(f) for f in glob(final, recursive=True)), ignore_index=True)
df.to_csv('final.csv')
print("All csv files are combined as one. :)")
